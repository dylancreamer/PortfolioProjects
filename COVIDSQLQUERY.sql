...
SELECT Location, Date, population, total_cases, (CONVERT(float, total_cases) / NULLIF(CONVERT(float, population), 0)) * 100 AS CovidAffectedPercentage
FROM pproject..['COVID Deaths$']
Where location like 'United States' AND total_cases IS NOT NULL AND continent is not null
order by 1,2

-- Looking at Countries with Highest Infection Rate compared to Population
SELECT Location, population, MAX(total_cases) as HighestInfectionCount, (CONVERT(float, MAX(total_cases)) / NULLIF(CONVERT(float, population), 0)) * 100 AS CovidAffectedPercentage
FROM pproject..['COVID Deaths$']
